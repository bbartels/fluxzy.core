...
namespace Fluxzy.Clients.H11
{
    ...
    public class Http11ConnectionPool : IHttpConnectionPool
    {
            ...
            try {
                ...
                try {
                    ...
                    exchange.RegisterConnectionDisposition(DisposeConnectionAfterExchange(
                        exchange, connection, maxConnection, abortRegistration,
                        cancellationToken, leaseAcquired));
                }
                catch (Exception ex) {
                    ...
                    if (deadConnSignal) {
                        ...
                        if (connection?.ReadStream != null) {
                            try {
                                await connection.ReadStream.DisposeAsync();
                            }
                            catch {
                        ...
                        if (connection?.WriteStream != null &&
                            !ReferenceEquals(connection.ReadStream, connection.WriteStream)) {
                            try {
                                await connection.WriteStream.DisposeAsync();
                            }
                            catch {
                                // transport may already be aborted
                            }
        ...
        private async Task DisposeConnectionAfterExchange(
            ...
            CancellationTokenRegistration abortRegistration, CancellationToken cancellationToken,
            bool releasePinnedLease)
        {
            ...
            try {
                ...
                // A shared connection that carried NTLM/Negotiate auth is bound to one
                // client's identity. Dedicated pools may retain it for that same client.
                ...
                // Send returns after response headers, so the disposition owns pool activity
                // and the pinned lease until the body is consumed and the connection is
                // recycled or closed.
                lock (_idleGate) {
                    _activeRequestCount--;
                    _lastActivity = _timingProvider.Instant();
